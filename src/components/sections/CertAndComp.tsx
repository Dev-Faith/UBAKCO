    ...
    <div className="w-full bg-[#EFEFEF] flex justify-center">
      ...
      <div className="py-[98px] grid lg:grid-cols-3 grid-cols-1 border-y-[1px] md:w-full border-[#CACACA] mt-[98px] gap-y-[80px] lg:gap-x-[30px] relative">
        {Certifications.map((cert, index) => (
          <div key={index} className={` w-[362px] md:w-full ${index !== 5 && "border-b-[1px]"} lg:border-b-0 border-[#CACACA] lg:border-b-0 pb-[40px] ${index !== 2 && index !== 5 ? "lg:border-r-[1px] lg:border-b-0 pb-[40px] lg:pb-0 border-b-[1px] pr-[28px]" : ""}`}>
            <Image
              ...
              height={48}
            />
            <p className="text-[20px] leading-[30px] md:text-[28px]/[42px] md:w-full pt-[24px]">{cert}</p>
          </div>
        ))}
